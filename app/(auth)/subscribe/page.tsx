...
type Step = 'pay' | 'create_account' | 'verify_otp' | 'activating';
...
export default function SubscribePage() {
  ...
  // Load PayPal SDK on mount (always — no auth gate)
  useEffect(() => {
  ...
  // Render PayPal buttons when SDK is ready and we're on the pay step
  useEffect(() => {
    if (step !== 'pay') return;
    ...
    const buttons = window.paypal.Buttons({
      ...
      createSubscription: async () => {
        ...
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/billing/guest-initiate`,
          {
            method: 'POST',
        ...
        if (!res.ok) {
          ...
          throw new Error((err as { message?: string }).message || 'Failed to create subscription');
        }
      ...
      onApprove: async (data: { subscriptionID: string }) => {
        ...
        if (session) {
          ...
          // Not logged in — collect account details now that payment is confirmed
          setPendingSubId(data.subscriptionID);
          setStep('create_account');
        }
      },

      onCancel: () => setError('Payment cancelled. Your plan is still available when you\'re ready.'),
      onError: (err: unknown) => {
  ...
  }, [sdkReady, selectedPlan, step, paypalRenderKey]);
  ...
  const handleCreateAccount = async () => {
    ...
    if (result.session) {
      // Email confirmed automatically — activate right away
  ...
  const activateAndRedirect = async () => {
    ...
    setStep('activating');
    try {
      await activateSubscription(pendingSubId);
      router.replace('/dashboard');
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Activation failed. Please contact support.');
      setStep('pay');
    ...
    <div className="flex-1 flex items-center justify-center p-4">
      ...
      >
        ...
        <div className="text-center mb-10">
          ...
          {step === 'pay' && (
            <>
              ...
              >
                ...
                <Link href="/login" className="text-[var(--color-accent)] font-medium hover:underline inline-flex items-center gap-1">
                  <LogIn size={13} />
                  Log in
                </Link>
              </motion.p>
          ...
          {step === 'create_account' && (
            <p className="text-[var(--color-text-secondary)] text-sm max-w-md mx-auto">
              Your payment was successful. Set up your account to access Vaultly™.
            </p>
          )}
        </div>
        ...
        {step !== 'activating' && (
          <div className="grid lg:grid-cols-2 gap-6">
            {/* Left column — plan selector + action area */}
            <div className="space-y-3">
              {/* Plan selector — only shown on pay step */}
              {step === 'pay' && PLANS.map((plan, i) => (
                <motion.button
                  key={plan.id}
                ...
                >
                  ...
                  <div className="flex items-center gap-3">
                    <div className={`flex-shrink-0 w-5 h-5 rounded-full border-2 flex items-center justify-center transition-all ${selectedPlan === plan.id ? 'border-[var(--color-accent)] bg-[var(--color-accent)]' : 'border-[var(--color-border)]'}`}>
                      {selectedPlan === plan.id && <Check size={11} className="text-[#0d0a06]" strokeWidth={3} />}
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="font-semibold text-[var(--color-text-primary)] text-base">{plan.name}</p>
                ...
                {step === 'pay' && (
                  <>
                    ...
                    <div className="relative">
                      ...
                      <div className="relative flex justify-center"><span className="bg-[var(--color-bg)] px-3 text-xs text-[var(--color-text-muted)]">or</span></div>
                    </div>
                ...
                {step === 'create_account' && (
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-3">
                    ...
                    <div className="relative">
                      ...
                      <input type="text" placeholder="Full name" value={signupName} onChange={(e) => setSignupName(e.target.value)} className="input-base pl-9 text-sm" autoComplete="name" />
                    </div>

                    <div className="relative">
                      <Mail size={15} className="absolute left-3 top-1/2 -translate-y-1/2 text-[var(--color-text-muted)]" />
                      <input type="email" placeholder="Email" value={signupEmail} onChange={(e) => setSignupEmail(e.target.value)} className="input-base pl-9 text-sm" autoComplete="email" />
                    </div>

                    <div className="relative">
                      <Lock size={15} className="absolute left-3 top-1/2 -translate-y-1/2 text-[var(--color-text-muted)]" />
                      <input type={showPass ? 'text' : 'password'} placeholder="Password (min. 8 characters)" value={signupPassword} onChange={(e) => setSignupPassword(e.target.value)} className="input-base pl-9 pr-10 text-sm" autoComplete="new-password" />
                    ...
                    <label className="flex items-start gap-2.5 cursor-pointer group">
                      <input type="checkbox" checked={agreedToTerms} onChange={(e) => setAgreedToTerms(e.target.checked)} className="mt-0.5 h-4 w-4 flex-shrink-0 rounded border-[var(--color-border)] bg-[var(--color-surface-2)] accent-[var(--color-accent)] cursor-pointer" />
                      <span className="text-xs text-[var(--color-text-muted)] leading-relaxed group-hover:text-[var(--color-text-secondary)] transition-colors">
                        I agree to the{' '}
                        <a href="/terms" target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()} className="text-[var(--color-accent)] hover:underline">
                          Terms &amp; Conditions
                        </a>
                      </span>
                    </label>
                    ...
                    <button onClick={handleCreateAccount} disabled={isCreatingAccount || !agreedToTerms} className="btn-primary w-full text-sm">
                      {isCreatingAccount
                        ? <><Loader2 size={15} className="animate-spin" /> Creating account…</>
                        : <>Create account &amp; access Vaultly™ <ArrowRight size={15} /></>}
                    </button>
                  </motion.div>
                )}
                  ...
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
                    ...
                    <div>
                      <input
                        ...
                        autoFocus
                      />
                      {otpError && <p className="text-[var(--color-danger)] text-xs mt-1 text-center">{otpError}</p>}
                    </div>

                    <button onClick={verifyOtp} disabled={verifying || otp.length !== 6} className="btn-primary w-full text-sm">
                      {verifying
                        ? <><Loader2 size={15} className="animate-spin" /> Verifying…</>
                        : <>Verify &amp; access Vaultly™ <ArrowRight size={15} /></>}
                    </button>
                    ...
                    <div className="space-y-2 text-center">
                      {rateLimit.isLockedOut ? (
                        <p className="text-xs text-[var(--color-danger)]">
                          ...
                          <span className="font-semibold tabular-nums">{rateLimit.lockoutMinutes}:{String(rateLimit.lockoutRemSeconds).padStart(2, '0')}</span>
                        </p>
                      ) : rateLimit.cooldownSeconds > 0 ? (
                        <div className="flex flex-col items-center gap-1">
                          <OtpCountdownTimer seconds={rateLimit.cooldownSeconds} />
                          <span className="text-[10px] text-[var(--color-text-muted)]">{rateLimit.attemptsLeft} attempt{rateLimit.attemptsLeft !== 1 ? 's' : ''} left</span>
                        </div>
                      ) : (
                        <button type="button" onClick={resendCode} disabled={resending || !rateLimit.canResend} className="inline-flex items-center gap-1.5 text-xs text-[var(--color-accent)] hover:underline disabled:opacity-50 disabled:cursor-not-allowed">
                          {resending
                      ...
                      <button type="button" onClick={() => { setStep('create_account'); rateLimit.clear(); }} className="block mx-auto text-xs text-[var(--color-text-muted)] hover:text-[var(--color-text-primary)]">
                        ← Back
                      </button>
                    </div>
                  </motion.div>
            ...
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 }} className="card p-6">
              <div className="flex items-center gap-2 mb-5">
                ...
                <h2 className="font-semibold text-[var(--color-text-primary)] text-sm">Everything included</h2>
              </div>
              <ul className="space-y-3">
                {FEATURES.map((feature, i) => (
                  <motion.li key={feature} initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.25 + i * 0.04 }} className="flex items-center gap-3">
